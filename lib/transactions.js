...
TxController.prototype.transformTransaction = function(transaction, options, callback) {
  ...
  if(transaction.height >= 0) {
    confirmations = this.node.services.myntd.height - transaction.height + 1;
  }
...
TxController.prototype.transformOutput = function(options, output, index) {
  ...
  if (output.address) {
    ...
    var address = myntcore.Address(output.address); //TODO return type from myntcore-node
...
TxController.prototype.transformMyntTransaction = function(transaction) {
